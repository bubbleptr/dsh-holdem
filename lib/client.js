...
function Table(props) {
  ...
  if (compact) {
    let playerRow2 = function(p) {
      const thinking = !!(p.isToAct && p.kind === "ai");
      const status = thinking ? thinkLabel || "\u601D\u8003\u4E2D" : p.talk || p.lastAction || (p.folded ? "\u5DF2\u5F03\u724C" : "");
      const won = !!winnerSeats[p.seat];
      return h(
        "div",
        {
          key: p.id,
          className: "hk-c-row" + (p.seat === 0 ? " me" : "") + (p.isToAct ? " toact" : "") + (p.folded ? " folded" : "") + (won ? " winner" : "") + (p.allIn ? " allin" : "")
        },
